...
var productExceptSelf = function(nums) {
    ...
    // 前缀数组 后缀数组
    // const n = nums.length;
    // const pre = new Array(n).fill(0); // 前缀
    // pre[0] = 1;
    // for(let i = 1; i < n; i++) pre[i] = pre[i - 1] * nums[i - 1];

    // const suf = new Array(n).fill(0); // 后缀
    // suf[n - 1] = 1;
    // for(let i = n - 2; i >= 0; i--) suf[i] = suf[i + 1] * nums[i + 1];

    // return new Array(n).fill(0).map((v, i) => pre[i] * suf[i]);

    // 暴力
    // return new Array(nums.length).fill(0).map((v, i) => nums.reduce((acc, cur, index) => {
