...
## [110. 平衡二叉树]((https://leetcode-cn.com/problems/ping-heng-er-cha-shu-lcof/))
...
function myBuildTree(preorder, inorder, preorder_left, preorder_right, inorder_left, inorder_right) {
    ...
    // 在中序遍历中定位根节点
    const inorder_root = inorder.indexOf(preorder[preorder_root]);
...
const pathSum = (root, targetSum) => {
    ...
    // map中，key存的是前缀和，value存的是前缀和的节点数量, 记录数量是因为有出现复数路径的可能。
    const map = new Map();
    let res = 0;
    dfs(root, 0);
    return res;
